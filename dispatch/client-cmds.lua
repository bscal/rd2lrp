...
RegisterCommand('gsrtest', function(source, args, rawCommand)
    if isCop then
...
AddEventHandler("311client", function(num, msg, loc)
    if (isCop) or (isEMS) then
...
AddEventHandler("911client", function(num, msg, loc)
    if (isCop) or (isEMS) then
        TriggerEvent('chat:addMessage', {
            color = {255, 55, 55},
            multiline = true,
...
AddEventHandler("callResponse", function(msg)
    TriggerEvent('chat:addMessage', {
        color = {51, 153, 255},
        multiline = true,
        args = {"[Dispatcher]",  msg}
...
AddEventHandler("gsrClient", function(copID)
    if (#gsr > 0) then
...
RegisterCommand('adminfix', function(source, args, rawCommand)
    if isCop then
...
RegisterCommand('dopen', function(source, args, rawCommand)
...
RegisterCommand('dclose', function(source, args, rawCommand)
    if (vRP.EXT.Garage:isInVehicle()) and hasKeys and isDriver() then
        local ped = GetPlayerPed(-1)
        local veh = GetVehiclePedIsIn(ped, false)
