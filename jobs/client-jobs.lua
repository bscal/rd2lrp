...
local jobLocations = {{name="Weed Delivery",blip=140,color=69,x=2488.0251464844,y=4961.0825195312,z=44.432537078858}}
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k, v in pairs(jobLocations) do
            ...
            if (dist < 2.0) then
                ShowInfoTextJobs("~y~Do job press ~p~H~y~.", .4, .8)
        ...
        for k, v in pairs(currentCocaineLocations) do
            ...
            if not IsPedInAnyVehicle(ped, false) then
                if dist < 2.0 and not isSellingCocaine then
                    ShowInfoTextJobs("~y~Sell cocaine ~p~H~y~.", .4, .8)
                ...
                elseif dist < 5.0 then
                    ...
                    if isSellingCocaine then
                        ShowInfoTextJobs("~y~You are selling cocaine please stay in the area.", .35, .8)
                    end
                end
            end
        end
        ...
        if weedStep > 0 and currentWeedLoc ~= nil then
            ...
            DrawMarker(1, currentWeedLoc.x, currentWeedLoc.y, currentWeedLoc.z - 1,0,0,0,0,0,0,0.8,0.8,0.8, 255, 55, 55, 155,0)
            if dist < 2.0 then
                if deliveringWeed > 0 then
                    vRP.EXT.GUI:setProgressBarValue("weed-delivery",deliveringWeed)
                else
                    ShowInfoTextJobs("~y~Deliver weed ~p~E~y~.", .4, .8)
