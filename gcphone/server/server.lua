...
function getSourceFromIdentifier(identifier, cb)
    TriggerEvent("vrp_getPlayers", function(users)
        cb(getUserByIdentifier(identifier).cid)
    end)
end
function getNumberPhone(identifier)
    local result = MySQL.Sync.fetchAll("SELECT vrp_character_identities.phone FROM vrp_character_identities WHERE vrp_character_identities.character_id = @identifier", {
...
function getIdentifierByPhoneNumber(phone_number) 
    local result = MySQL.Sync.fetchAll("SELECT vrp_character_identities.character_id FROM vrp_character_identities WHERE vrp_character_identities.phone = @phone_number", {
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
    ...
    if myPhoneNumber == '0' or myPhoneNumber == nil then
        ...
        until id == nil
        MySQL.Async.insert("UPDATE vrp_character_identities SET phone = @myPhoneNumber WHERE character_id = @identifier", { 
...
function getMessages(identifier)
    local result = MySQL.Sync.fetchAll("SELECT phone_messages.* FROM phone_messages LEFT JOIN vrp_character_identities ON vrp_character_identities.character_id = @identifier WHERE phone_messages.receiver = vrp_character_identities.phone", {
        ['@identifier'] = identifier
   })
    return result
...
AddEventHandler('vrp:playerReady',function(user)
    local sourcePlayer = user.source
    local identifier = user.cid
